...
동작:
  1) SITE_BASE_URL 을 비워서 빌드 → 이미지/notice 경로가 상대(/images, /notice.json)가 됨
     (로컬 서버 루트에서 이미지가 깨지지 않도록)
  2) 최신 리포트를 _localtest/index.html 로, images/·notice.json 을 루트에 배치
...
def build_and_stage(date: str | None) -> None:
    ...
    if notice.exists():
        shutil.copy2(notice, SITE / "notice.json")
