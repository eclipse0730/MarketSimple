...
# 마스코트 위젯. 캐릭터·대사·이미지는 모두 배포 사이트의 characters.json 에서 읽고
# 동작은 mascots.js 가 담당한다 → 재빌드 없이 두 파일만 고치면 갱신된다.
...
# ──────────────────────────────────────────────
# 시장 진단(대형주/중소형 강도 · 과열 신호) 섹션 임계값
# ──────────────────────────────────────────────
# 시가총액 구간(원). 대형/중형/소형으로 나눠 구간별 평균 등락률·종목 수를 본다.
CAP_LARGE = 1_000_000_000_000   # 1조 이상: 대형주
CAP_MID = 100_000_000_000       # 1천억 이상 1조 미만: 중형주 (그 아래는 소형주)
# 상한가/하한가 근접 판정(%). 한국 상하한가 ±30% 에 근접한 종목을 과열/패닉 신호로 카운트.
LIMIT_UP_RATE = 29.0
LIMIT_DOWN_RATE = -29.0
